...
#     username = request.session.get("username", "")  # Assuming you have "username" in the session
...
#     # print (user_orders[0].items_json)
...
def add_rating(request, order_id, product_key):
    if request.method == 'POST':
        ...
        username = request.user.username  # Adjust this based on your authentication logic
        rating = int(request.POST.get('rating', 0))

        print(product, username, rating, "eee")
        ProductReview.objects.create(product=product, username=username, rating=rating)
...
def my_logout_page(request):
    ...
    if "adminusername" in request.session:
        request.session.pop("adminusername")
...
def adminpage(request):
    if request.session.get("adminusername"):
        ...
        ship_from_count = Orders.objects.values('shipfrom').distinct().count() or 0

        # Calculate total number of orders
        total_orders = Orders.objects.count() or 0
